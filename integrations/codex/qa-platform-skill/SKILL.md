...
description: Scan Python, Spring Boot/Java, Go, Node, and other common projects for HTTP and WebSocket interfaces; map project-configured service topology into stable external route prefixes and service/business API directories; deterministically convert OpenAPI, Swagger, AsyncAPI, JavaDoc, and annotation contracts into complete executable definitions including recursive request fields and response schemas; discover safe reusable API templates from project request and gateway configuration; read project-configured flow documents; generate editable module JSON files; and build or preview-publish a validated ZIP for qa-platform. Use when asked to inventory interfaces, bootstrap API tests, organize APIs into directory groups, generate documented flows, decide update versus new test version, configure qa-platform assets, or import scan results.
---
...
- For multi-service projects, configure `.qa-platform.json.service_topology.services` with each service's `source_roots`, external `route_prefix`, `group_path`, application/server facts, and gateway mapping. This project-owned topology is authoritative when a Controller and its `boot` configuration live in sibling modules. Map each source file to the most specific `source_root`, prefix Spring HTTP/WS routes with that service's external route, and organize APIs as `service group_path + controller/business directory`. Ordered `api_grouping.rules` still have highest directory precedence.
- For Spring Boot/Cloud routes without an authoritative configured service prefix, inspect module-scoped `application*.yml`, `application*.yaml`, `application*.properties`, and `bootstrap*` files. Apply literal `server.servlet.context-path`, `server.context-path`, `spring.mvc.servlet.path`, or `spring.webflux.base-path` values to Spring HTTP/`@MessageMapping` paths, retain configuration source references, and warn when profile candidates disagree. Leave OpenAPI/Swagger paths unchanged because those documents may already include the deployed prefix.
...
- Emit an editable module bundle as the primary scan result: `manifest.json`, `project.json`, `api_templates.json`, `assertion_definitions.json`, `inventory.json`, `flow_documents.json`, `api.json`, `flow.json`, and `plans.json`. Each API record carries a canonical `group_path`; the current importer creates the directory chain from that field, so do not invent an unsupported standalone `api_groups.json` ZIP module. Also emit `qa-platform-import.json` as a compatibility snapshot. Validation and ZIP construction must read the module directory so later AI/reviewer edits to one module are authoritative.
...
- Use `key` as the only public HTTP/WS interface identity: `http:<METHOD>:<path>` for HTTP and `ws:<path-or-url>` for WebSocket. `group_path` is only a display/organization attribute and must never be folded into `key`. The scanner may use the same route key internally to merge duplicate source references, but it does not emit a separate `identity_key` or `business_key` on interface records.
- Construct imported API parameters only with qa-platform's executable `path`/`query`/`header`/JSON `body` model. Materialize every resolved `object.properties` tree as recursive `children`; child nodes omit `in` and inherit the parent location. Keep arrays as `items` rather than inventing array-index paths. Every root and child parameter must have a non-empty description and a safe example. Preserve documented values first; when source has no business description, derive a readable identifier label or state explicitly that the meaning needs review instead of emitting low-value text such as “路径参数 `toolCode`（string）”. Emit `default` only when source/config explicitly supports it. Do not copy raw OpenAPI Parameter Objects or pretend multipart/scalar bodies are executable.
...
- For Java/Spring parameters, read method JavaDoc `@param` tags and prefer parameter annotations (`@Schema`, `@Parameter`) before JavaDoc. For DTO fields, preserve `@Schema`, `@Parameter`, `@ApiModelProperty`, `@JsonPropertyDescription`, explicit Bean Validation messages, field JavaDoc, and line comments in that order. Strip comments before parsing declarations, resolve nested classes/records/enums recursively, and preserve enum fields as `string` with `enum`. Apply the same recursive schema to request body parameters and response fields. When OpenAPI/Swagger response data is absent, use the Spring method return type as a secondary response contract source; unwrap `ResponseEntity<T>`/`Optional<T>`, expand DTOs, and represent common `R<T>`/`Result<T>` wrappers as reviewable `code`/`data` fields with warnings where the wrapper is inferred.
...
- Give every API a specific display name of at most 120 characters. Prefer the first human-facing sentence of an OpenAPI summary/operation ID or Spring controller/interface and method JavaDoc; keep longer explanatory prose in `description`. Apply the same rule to Spring `WebSocketConfigurer#addHandler` registrations. Preserve that source-derived name during localization. Without comments, use a meaningful Java identifier or route token before a generic fallback. For a residual collision, add a bounded HTTP/WS method and request-target suffix deterministically instead of leaving repeated names such as “查询接口”.
...
4. Review the generated modules before importing:
   ...
   - `api.json`, `flow.json`, and `plans.json` are independently editable. Review every API's `group_path`; edit it in `api.json` when the inferred directory is wrong. If AI improves a documented flow, edit `flow.json`; do not hand-edit the compatibility aggregate.
   ...
   - `service_topology` is project-owned runtime/deployment metadata. Verify every service `source_roots`, `route_prefix`, `group_path`, application name, server port/context path, and gateway route before accepting stable API keys.
   ...
   - Review directory warnings: configured `api_grouping.rules` are authoritative; otherwise a configured service `group_path` is combined with OpenAPI/Swagger extensions, hierarchical tags, or controller/business hints. URL-path grouping is the last fallback.
...
9. Create the project-local configuration before generating an import package:
   ...
   This creates `.qa-platform.json` with project metadata, required `variables.base_url`, API templates, an empty `api_grouping` rule set, an empty `service_topology`, success conditions, flow-document sources, OpenAPI sources/runtime settings, detected language, optional `package_version` override, and versioned artifact storage. For a multi-service project, fill `service_topology` before scanning; use ordered `api_grouping.rules` for exceptions or an explicitly curated business directory. The project variable is an `ip:port` value because qa-platform adds `http://` when resolving it. The top-level `base_url` remains the qa-platform service location. The file never contains a token.
...
API directory grouping rules:

- Keep `group_path` canonical: `/` for the root or `/一级目录/二级目录`; it is independent of the stable API `key`.
- Configure deterministic rules in `.qa-platform.json` with `api_grouping.default_path` and ordered `api_grouping.rules`. A rule has `group_path` and a `match` object supporting `protocol`, `method`/`methods`, `key`/`keys`, `path`/`paths`, `path_prefix`, `path_regex`, `operation_id`, `business_key`, `service`, and `tags`. The first matching rule wins; multiple matches produce a warning.
- If no rule matches, use OpenAPI/Swagger operation or path extensions (`x-qa-platform-group-path`, `x-api-group-path`, `x-group-path`), then tag extensions or hierarchical tags such as `用户服务/用户管理`, then controller/module/business heuristics, and finally meaningful URL path segments. Uncertain inference must remain visible in `warnings`.
- Do not create a separate empty-directory asset for the current ZIP contract. Importing an API with `/用户服务/用户管理` automatically creates each missing directory in qa-platform; empty directories remain a platform-side CRUD concern.
