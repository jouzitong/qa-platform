...
- Read `.qa-platform.json` as the project-owned bootstrap contract. It may define `project`, `variables`, reusable `api_templates`, `success_assertions`, `flow_documents`, `openapi`, language, version, storage, and qa-platform publishing settings. Keep these values non-secret.
...
6. Summarize counts for HTTP interfaces, WS interfaces, API templates, success conditions, flow documents, features, test cases, flows, test plans, warnings, gateway evidence, and unresolved assumptions. Highlight low-confidence items, generated placeholder descriptions/examples, and missing schemas.
...
3. Structured project flow-document blocks for flow order and intent.
4. Frontend router and page metadata.
5. Controller/service names, prose flow documents, documentation, and comments.
6. Heuristic grouping by URL path or page route.
